...
class SeparableFilterNd(SeparablePoolNd):
    ...
    def forward(self, x: torch.Tensor, axes=None, same=True, padder: Padder = None):
        if same and padder is not None:
            # same padding
            same_pad_width = self.kernel_size.map(_same_padding_pair)
            padder = Padder(pad_width=same_pad_width,
...
class MyTestCase(unittest.TestCase):
    def test_uniform(self):
        for n in range(1, 10):
            ...
            for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
                y_ti = SeparableFilterNd(np.ones(n) / n)(x, same=True, padder=Padder(mode=ti_mode))
    ...
    def test_conv(self):
        for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
            for ndim in range(1, 5):
                ...
                y_ti = SeparableFilterNd(kernels)(x, same=True, padder=Padder(mode=ti_mode))
    ...
    def test_wrapper_1(self):
        ...
        y1 = gf_1(x, padder=Padder(mode="reflect"))
        y2 = gf_2(x, padder=Padder(mode="reflect"))
        y = gp(x, padder=Padder(pad_width=4, mode="reflect"))
        ...
        for truncate in range(2, 10, 2):
            for order in range(6):
                for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
                    ...
                    y_ti = gf1(x, axes=None, padder=Padder(mode=ti_mode))
                    y_ti = y_ti.numpy()
    ...
    def test_precision_1(self):
        ...
        for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
            ...
            y2 = pool_to_filter(SeparablePoolNd, same=True)(w)(x, padder=Padder(mode=ti_mode)).numpy()
    ...
    def test_average_1(self):
        ...
        for kernel_size in range(3, 15, 2):
            ...
            for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
                y_ti = UniformFilterNd(kernel_size=kernel_size)(x, padder=Padder(mode=ti_mode)).numpy()
