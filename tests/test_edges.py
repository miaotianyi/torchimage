...
class MyTestCase(unittest.TestCase):
    def test_edge_filters(self):
        for ti_cls, ski_h, ski_v, ski_mag in [
            ...
            actual_h = ti_filter.horizontal(x1, same=True, padder=Padder(mode="symmetric")).numpy()
            expected_h = ski_h(x2)
            with self.subTest(shape=x1.shape, detector=ti_cls, task="horizontal"):
                self.assertLess(np.abs(actual_h - expected_h).max(), 1e-12)
            actual_v = ti_filter.vertical(x1, same=True, padder=Padder(mode="symmetric")).numpy()
            expected_v = ski_v(x2)
            with self.subTest(shape=x1.shape, detector=ti_cls, task="vertical"):
                self.assertLess(np.abs(actual_v - expected_v).max(), 1e-12)

            actual_mag = ti_filter.magnitude(x1, same=True, padder=Padder(mode="symmetric"), epsilon=0.0).numpy()
    ...
    def test_ndimage_sobel(self):
        for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
            ...
            y_actual = sobel.component(x1, -1, -2, same=True, padder=Padder(mode=ti_mode)).numpy()
            ...
            y_actual = sobel.component(x1, -2, -1, same=True, padder=Padder(mode=ti_mode)).numpy()
    ...
    def test_ndimage_prewitt(self):
        for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
            ...
            y_actual = prewitt.component(x1, -1, -2, same=True, padder=Padder(mode=ti_mode)).numpy()
            with self.subTest(shape=x1.shape, edge_axis=-1, mode=ti_mode, name="sobel"):
                self.assertLess(np.abs(y_expected - y_actual).max(), 1e-12)

            y_expected = ndimage.prewitt(x2, axis=-2, mode=ndimage_mode)
            y_actual = prewitt.component(x1, -2, -1, same=True, padder=Padder(mode=ti_mode)).numpy()
            with self.subTest(shape=x1.shape, edge_axis=-2, mode=ti_mode, name="sobel"):
                self.assertLess(np.abs(y_expected - y_actual).max(), 1e-12)
    ...
    def test_gaussian_gradient(self):
        ...
        for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
            ...
            actual = gg.magnitude(x1, axes=None, padder=Padder(mode=ti_mode), epsilon=0.0).numpy()
    ...
    def test_laplace_2(self):
        ...
        for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
            ...
            x2 = x1.numpy()
            y_actual = laplace_2(x1, axes=None, padder=Padder(mode=ti_mode), ).numpy()
    ...
    def test_log(self):
        ...
        for ti_mode, ndimage_mode in NDIMAGE_PAD_MODES:
            ...
            x2 = x1.numpy()
            y_actual = edges.LaplacianOfGaussian(kernel_size=ks, sigma=sigma)(x1, padder=Padder(mode=ti_mode)).numpy()
