...
def pool_to_filter(cls=SeparablePoolNd, same=True):
    class Wrapper(cls):
        ...
        def forward(self, x, *args, **kwargs):
            if "padder" in signature(super().forward).parameters:
                # pooling module supports forward inference with a padder
                # this is common for separable convolution
                ba = signature(super(Wrapper, self).forward).bind(x, *args, **kwargs)
            ...
            else:  # generic pooling, doesn't support separable padding
                padder = kwargs.pop("padder") if "padder" in kwargs else None
                if padder is not None:
                    if same:  # make same padder
                        ...
                        padder = Padder(pad_width=same_pad_width,
                                        ...
                                        stat_length=padder.stat_length.data)
                    x = padder(x)
                ba = signature(super().forward).bind(x, *args, **kwargs)
            return super().forward(*ba.args, **ba.kwargs)
